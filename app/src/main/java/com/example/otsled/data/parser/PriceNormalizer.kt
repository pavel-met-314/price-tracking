...
object PriceNormalizer {
    ...
    private val articlePattern = Regex("""Артикул\s*\d+""", setOf(RegexOption.IGNORE_CASE, RegexOption.UNICODE_CASE))
    private val volumePattern = Regex("""\d+\s*мл\.?""", setOf(RegexOption.IGNORE_CASE, RegexOption.UNICODE_CASE))
    private val rubPricePattern = Regex("""(\d[\d\s\u00A0]{0,10})\s*руб\.?""", setOf(RegexOption.IGNORE_CASE, RegexOption.UNICODE_CASE))
