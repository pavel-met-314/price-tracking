...
object PriceNormalizer {
    ...
    private val articlePattern = Regex("""(?iu)Артикул\s*\d+""", RegexOption.IGNORE_CASE)
    private val volumePattern = Regex("""(?iu)\d+\s*мл\.?""", RegexOption.IGNORE_CASE)
    private val rubPricePattern = Regex("""(?iu)(\d[\d\s\u00A0]{0,10})\s*руб\.?""", RegexOption.IGNORE_CASE)
