...
data class TrackedProductEntity(
    ...
    /**
     * Название, которое пользователь вписал сам. Пустое — показываем то, что прочитано со
     * страницы: ручная правка перекрывает разбор, но не затирает его — следующая проверка
     * обновит `title`, оставив override нетронутым.
     */
    val titleOverride: String? = null,
    /** Отпечаток последней удачной разметки (см. domain/LayoutFingerprint). */
    val lastFingerprint: String? = null,
    /** Что перестало находиться на странице; пустая строка — вёрстка ведёт себя нормально. */
    @ColumnInfo(defaultValue = "''")
    val lastLayoutNote: String = "",
)
